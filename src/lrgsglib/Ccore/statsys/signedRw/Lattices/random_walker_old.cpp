...
class RandomWalker {
public:
    RandomWalker(py::list edge_list, py::list sign_list, int width, int height, int coordination_number)
...
private:
    ...
    void initializeNeighborsAndSigns(py::list edge_list, py::list sign_list) {
        for (int i = 0; i < len(edge_list); ++i) {
            ...
            int sign = py::extract<int>(sign_list[i]);
...
BOOST_PYTHON_MODULE(random_walker) {
    ...
    py::class_<RandomWalker>("RandomWalker", py::init<py::list, py::list, int, int, int>())
